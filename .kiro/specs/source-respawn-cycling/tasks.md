...
- [ ] 1. Add respawn config fields to `SourceFieldConfig`
  - [ ] 1.1 Add `respawn_enabled: bool`, `min_respawn_cooldown_ticks: u32`, `max_respawn_cooldown_ticks: u32` to `SourceFieldConfig` in `src/grid/world_init.rs`
  ...
  - [ ] 1.2 Add respawn validation to `validate_source_field_config` in `src/grid/world_init.rs`
...
- [ ] 2. Add `RespawnEntry` and `RespawnQueue` data structures
  - [ ] 2.1 Create `RespawnEntry` struct and `RespawnQueue` in `src/grid/source.rs`
  ...
  - [ ] 2.2 Add `respawn_queue: RespawnQueue` field to `Grid` in `src/grid/mod.rs`
...
- [ ] 3. Implement depletion event detection in `run_emission`
  - [ ] 3.1 Add `DepletionEvent` struct and `iter_mut_with_ids` method to `SourceRegistry` in `src/grid/source.rs`
    - `DepletionEvent`: `source_id: SourceId`, `field: SourceField`, `tick: u64`
    - `iter_mut_with_ids`: yields `(SourceId, &mut Source)` in slot order
    - _Requirements: 1.1_
  - [ ] 3.2 Modify `run_emission` to detect depletions and return `SmallVec<[DepletionEvent; 8]>`
...
- [ ] 5. Implement respawn queue processing and source spawning
  - [ ] 5.1 Implement `run_respawn_phase` function in `src/grid/source.rs`
...
- [ ] 6. Integrate into tick orchestration
  - [ ] 6.1 Modify `run_emission_phase` in `src/grid/tick.rs` to process depletion events
  ...
  - [ ] 6.2 Call `run_respawn_phase` after emission in `run_emission_phase` or `TickOrchestrator::step`
...
- [ ] 8. Update configuration documentation
  - [ ] 8.1 Update `example_config.toml` with new respawn fields
    ...
    - _Requirements: 9.1_
  - [ ] 8.2 Update `format_config_info()` in `src/viz_bevy/setup.rs`
    - Display the new respawn fields in the info panel for both heat and chemical source configs
    - _Requirements: 9.3_
  - [ ] 8.3 Update `config-documentation.md` steering rule
