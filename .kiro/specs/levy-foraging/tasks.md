...
- [-] 1. Add tumble state fields to Actor and update ActorConfig
  - [-] 1.1 Add `tumble_direction: u8` and `tumble_remaining: u16` to the `Actor` struct in `src/grid/actor.rs`
  ...
  - [ ] 1.2 Add `levy_exponent: f32` (default 1.5) and `max_tumble_steps: u16` (default 20) to `ActorConfig` in `src/grid/actor_config.rs`
    - Update the `Default` impl
    - _Requirements: 7.1, 7.2, 7.3, 7.4_
  - [ ] 1.3 Add `seed: u64` field to the `Grid` struct in `src/grid/mod.rs`
    ...
    - _Requirements: 6.1_
  - [ ] 1.4 Add config validation for `levy_exponent > 1.0` and `max_tumble_steps >= 1` in `validate_world_config` in `src/io/config_file.rs`
...
- [~] 3. Implement `sample_tumble_steps` and `direction_to_target` helpers
  - [ ] 3.1 Add `rand_chacha = "0.3"` dependency to `Cargo.toml` (if not already present; `rand` should already be a dependency)
    - _Requirements: 6.1_
  - [ ] 3.2 Implement `sample_tumble_steps(rng, alpha, max_steps) -> u16` in `src/grid/actor_systems.rs`
  ...
  - [ ] 3.3 Implement `direction_to_target(cell_index, direction, w, h) -> Option<usize>` in `src/grid/actor_systems.rs`
...
- [~] 4. Rewrite `run_actor_sensing` with Lévy flight logic
  - [ ] 4.1 Update `run_actor_sensing` signature to accept `&mut ActorRegistry`, `&ActorConfig`, and `&mut impl Rng`
    - Change `actors: &ActorRegistry` to `actors: &mut ActorRegistry`
    - Add `config: &ActorConfig` and `rng: &mut impl Rng` parameters
    - _Requirements: 9.1, 9.2, 9.3_
  - [ ] 4.2 Implement break-even threshold evaluation and tumble state machine in `run_actor_sensing`
  ...
  - [ ] 4.3 Update all `run_actor_sensing` call sites to pass the new parameters
...
- [~] 5. Wire per-tick RNG into the tick orchestrator
  - [ ] 5.1 Update `TickOrchestrator::step` signature to accept `tick: u64` in `src/grid/tick.rs`
  ...
  - [ ] 5.2 Update all `TickOrchestrator::step` call sites to pass the tick number
...
- [~] 7. Update configuration documentation
  - [ ] 7.1 Update `example_config.toml` with `levy_exponent` and `max_tumble_steps` under `[actor]` with explanatory comments
    - _Requirements: 10.1_
  - [ ] 7.2 Update `format_config_info()` in `src/viz_bevy/setup.rs` to display `levy_exponent` and `max_tumble_steps`
    - _Requirements: 10.2_
  - [ ] 7.3 Update `README.md` ActorConfig parameter table with `levy_exponent` and `max_tumble_steps`
    - _Requirements: 10.4_
  - [ ] 7.4 Update `.kiro/steering/config-documentation.md` ActorConfig table with `levy_exponent` and `max_tumble_steps`
    - _Requirements: 10.3_
