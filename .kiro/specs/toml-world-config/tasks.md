...
- [x] 1. Add dependencies and serde derives to existing config structs
  - [x] 1.1 Add `serde`, `toml` dependencies to `Cargo.toml`
    - Add `serde = { version = "1", features = ["derive"] }` and `toml = "0.8"` to `[dependencies]`
    - _Requirements: 1.1, 1.4_
  - [x] 1.2 Add `Serialize`, `Deserialize`, `Default` derives to `GridConfig`
  ...
  - [x] 1.3 Add `Serialize`, `Deserialize` derives to `WorldInitConfig` and `SourceFieldConfig`
  ...
  - [x] 1.4 Add `Serialize`, `Deserialize`, `Default` derives to `ActorConfig`
...
- [x] 2. Create the `src/io/` module with config error type
  - [x] 2.1 Create `src/io/mod.rs` and `src/io/config_error.rs`
...
- [-] 3. Implement config file loader and serializer
  - [x] 3.1 Create `src/io/config_file.rs` with `WorldConfig`, `BevyExtras`, `BevyWorldConfig` structs
  ...
  - [x] 3.2 Implement `load_world_config`, `load_bevy_config`, `to_toml_string`, and `validate_world_config`
...
- [x] 5. Implement CLI argument parser
  - [x] 5.1 Create `src/io/cli.rs` with `CliArgs` struct and `parse_cli_args` function
...
- [x] 6. Integrate config loading into both binaries
  - [x] 6.1 Update `src/main.rs` to use CLI parser and config loader
  ...
  - [x] 6.2 Update `src/bin/bevy_viz.rs` to use CLI parser and config loader
